...
import com.udacity.geofence.basics.util.createChannel
    ...
    */
    ...
    override fun onNewIntent(intent: Intent?) {
        ...
        if (extras != null) {
            if (extras.containsKey(Constant.EXTRA_GEOFENCE_INDEX)) {
                mainViewModel.updateHint(extras.getInt(Constant.EXTRA_GEOFENCE_INDEX))
                checkPermissionsAndStartGeofencing()
            }
        }
    }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (
            ...
            // Permission denied.
            Snackbar.make(
                binding.contentMain.constraintParent,
    ...
    private fun checkPermissionsAndStartGeofencing() {
        if (mainViewModel.geofenceIsActive()) return
    ...
    private fun addGeofenceForClue() {
        if (mainViewModel.geofenceIsActive()) return
        val currentGeofenceIndex = mainViewModel.nextGeofenceIndex()
        if (currentGeofenceIndex >= Constant.NUM_LANDMARKS) {
            removeGeofences()
            mainViewModel.geofenceActivated()
            return
        }
